...
- PR 본문에 그 작업이 닫는 이슈를 **`Closes #N`**으로 적는다. 진행 상태는 이슈가 소유하는데 손으로 닫으면 잊는다 — 실제로 걷는 뼈대 이슈가 끝난 뒤로도 열려 있었다. 남는 항목이 있으면 닫지 말고 **무엇이 남았는지 코멘트로** 적는다. "거의 끝났다"고 닫는 것이 파일에 `status: done`을 적던 것과 같은 실수다.
