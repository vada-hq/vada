...
진행 상태(착수·검토·완료)는 GitHub Issue와 PR이 소유한다. 파일에 상태를 복사하지 않는다. **`just validate-screens`가 이것을 강제한다** — 화면 정본에 `status:`를 적으면 실패한다. 전에는 손으로 적은 `status: done`을 현황판이 그대로 읽었고, 그 자기 신고가 "3단계 5/6 완료"를 만들었다. 그중 사람이 브라우저에서 본 것은 0개였다.
...
**jsdom은 픽셀을 그리지 않는다.** 레이아웃 엔진이 없어 CSS Grid를 계산하지 않고, z-index도 히트 테스트도 없다. 그래서 `user.click()`은 **가려진 요소에도 항상 성공**하고, 나란한 두 입력칸의 높이가 어긋나도 통과한다. 실제로 화면 테스트 110건이 초록인 채로 사람이 브라우저를 열었더니 드롭다운이 안 열렸다 — 열리기는 하는데 대화상자가 클릭을 가로채고 있었다.

레이아웃·클릭·브라우저 입력 규칙은 `just test-web-browser`가 본다. **부탁이 아니라 강제다** — 화면 정본에 `route`가 있는데 `apps/web/e2e/`에 그 화면 id를 다루는 spec이 없으면 `just validate-screens`가 실패한다. 규칙을 글로만 적었을 때 어떻게 되는지는 이 저장소가 이미 보여줬다: 완료의 정의와 "파일에 상태를 복사하지 않는다"가 둘 다 문서에 있었고 둘 다 안 지켜졌다.
...
- PR 본문에 **`Closes #N`** 또는 **`이슈 없음: <이유>`** 중 하나를 적는다. CI가 확인한다(`ownership`). 진행 상태는 이슈가 소유하는데 손으로 닫으면 잊는다 — 이 규칙을 글로만 적었더니 **규칙을 만든 PR 다음부터 다섯 건이 연속으로 어겼다.** 남는 항목이 있으면 닫지 말고 **무엇이 남았는지 코멘트로** 적는다. "거의 끝났다"고 닫는 것이 파일에 `status: done`을 적던 것과 같은 실수다.
...
**사람에게 보내는 말은 두 가지뿐이다. 섞지 마라.**

| 사람이 할 일 | 형식 |
| --- | --- |
| **결정** | 선택지를 만들어 묻는다. 산문에 질문을 섞지 않는다. 각 선택지에 그것을 고르면 무슨 일이 생기는지와 권장안을 적는다 |
| **확인** (브라우저·명령·머지) | **`지금 하실 것 하나`** 로 한 가지만 적는다 |

확인 요청은 한 번에 하나다. 여러 개를 나열하면 어디서부터 할지 몰라 멈춘다. 그리고 **무엇을 보면 되는지**를 한 문장으로 적는다 — "확인해 주세요"만으로는 기준이 없다. 할 일이 없으면 없다고 적고 다음에 무엇을 할지 밝힌다.

승인을 요청할 때 계약이나 변경 목록을 나열하지 마라. 읽어도 판단할 수 없다. **판단할 것을 한두 개로 좁혀서** 내민다. 나머지가 정본과 맞는지는 계약을 쓴 쪽이 확인할 몫이다.
