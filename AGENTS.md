...
- 사람에게 승인을 요청하는 것은 제품 의미, 비용·일정, 보안·운영 위험, 비가역성 또는 여러 타당한 대안 사이의 선택이 결과를 실질적으로 바꿀 때로 제한한다. 승인된 기준과 저장소 증거에서 단일 권장안이 도출되는 가역적 기술 구조·문서 정리·의존관계 보완은 총괄 에이전트가 결정하고 근거와 검증 결과를 기록한 뒤 계속 진행한다. 스키마가 승인 근거를 요구하면 제품 책임자의 이 위임 원칙과 해당 작업의 증거를 함께 연결하며, 의식적인 확인 질문을 만들지 않는다.
