...
## 6. 검토가 끝난 뒤 (2026-08-31 덧붙임)

판정은 **승인 보류**였고, **검증 가능한 지적이 또 전부 사실이었다.** 하나하나 재현하고
고쳤다. 재현되지 않은 지적은 없었다.

### 출시를 막던 것 셋

**1. 일반 부원이 초대 코드를 읽었다.** 정책은 `org.invite`를 회장단만이라 하는데
읽는 자리만 `member`로 매여 있었다. 다시 만들지는 못해도 **지금 가입 열쇠를 복사해
나눌 수 있었다.** '초대는 회장단만'이 다시 만드는 것만 뜻한 것이 아니다.

**2. 공개 영수증을 남의 학번으로 가져갈 수 있었다.** 계약이 `(토큰, 학번)`이 같으면
첫 영수증을 그대로 주라고 적어 둔 탓이다. 공유 QR과 남의 학번만 있으면 그 사람의
영수증을 받아 이름과 납부 상태를 열었다.

**학번은 아무나 적을 수 있는 값이고 영수증은 그 사람만 가져야 하는 값이다.**
앞의 것으로 뒤의 것을 가리면 안 된다. 갈래를 둘로 나눴다 — 되풀이는 보내는 쪽이 만든
`Idempotency-Key`가 가리고(같은 기기의 두 번 누름·새로고침은 같은 결과를 본다),
같은 사실이 이미 있는지는 `conflict.naturalKey`가 표에서 가린다. 막혔을 때 **돌려주는
것은 없다** — 409로 이미 냈다는 사실만 알린다.

**3. 법정 기록에 '누가'가 없었다.** 감사 미들웨어가 `c.get('userId')`를 읽는데
**아무도 그것을 채우지 않았다** — 셸 자리에 있던 미들웨어가 채우고 있었는데 자리를
계약이 만드는 층으로 옮기면서 함께 사라졌다. 검사도 `orgId: null`을 승인하고 있었다.
게다가 표에 쓰는 sink가 아예 없었고, 사용자를 지우면 행위자가 `SET NULL`로 사라졌다.

고친 것: 보낸 사람을 요청 시작에 한 번 확정해 문맥에 담고, 4xx도 실패로 표시하고,
`databaseAudit`으로 실제 표에 쓰고, 감사와 권한 변경이 사용자를 **가리키지 않게** 했다
(권한 변경의 조직에 먼저 쓴 방식과 같다).

### 남의 학생회가 새고 있었다

지적 중 가장 조용한 것이었다. 뿌리 표는 `orgId`로 걸렀지만 **이어 붙인 표는 자기 조직을
확인하지 않았고**, 표가 남의 조직 줄을 가리키는 것을 막는 것도 없었다. 남의 조직 사람이
우리 부서를 가리키면 그 이름과 학부와 학년이 우리 조직도에 그려졌다. 행사의 담당도 같다.

재현한 뒤 **벽을 두 겹으로** 세웠다.

- 표가 막는다 — `(orgId, id)` 짝을 가리키는 복합 외래 키. 남의 조직 부서·사람을
  가리키는 줄은 아예 들어오지 못한다.
- 조회도 본다 — 이어 붙일 때 조직을 함께 건다. 제약이 언젠가 느슨해져도 새지 않는다.

두 번째 벽을 재려면 첫 번째를 잠깐 떼야 한다(검사가 그렇게 한다). 떼지 않으면 그 상태를
만들 수조차 없어 **조회가 정말 거르는지 알 수 없다.**

### 그 밖에 고친 것

- **`matchRoute`가 자리별이 아니라 총 개수를 봤다.** `/a/{x}/c`와 `/a/b/{y}`처럼 개수가
  같으면 계약에 적힌 차례가 권한을 정한다 — 설명과 구현이 달랐다. 앞에서부터 견주게 고쳤다.
  잘못된 `%` 인코딩이 500을 내던 것도 함께 막았다.
- **보낸 사람을 요청마다 한 번만 묻는다.** 공개 자리 한 번에 두 번 부르고 있었다.
- **`event.list`가 계약 밖의 단계에 500을 냈다.** 500은 안쪽 사정을 흘리고, 받는 쪽은
  '내가 잘못 물었다'와 '서버가 고장났다'를 가릴 수 없다. 422로 답한다.
- **`privacyConsent`가 `false`여도 통과했다.** `required`는 '값이 있는가'만 묻고 켜고 끄는
  칸에서는 **꺼짐도 값이다.** `mustBeTrue`를 어휘로 만들고 판정기가 보게 했다.
- **`npm run dev`가 없는 파일을 가리켰다.** 지웠다 — 도는 서버가 아직 없다는 것이
  사실이고, 있는 척하는 스크립트는 그 사실을 가린다.

### 아직 안 고친 것

- **429가 계약에만 있다.** 검토가 계층형 제한(IP·토큰 해시·학번 해시)과 캠퍼스 NAT
  문제를 짚었다. 참석·설문 표를 만들 때 함께 한다.
- **멱등이 동시 요청을 막지 못한다.** 같은 키의 두 요청이 둘 다 miss한 뒤 실행된다.
  프로세스 안 저장을 표로 옮기면서 유일 제약으로 막는다.
- **트랜잭션 경계가 없다.**
- **`maskSecrets`가 자리 규칙으로 지운다.** 정적 경로 이름까지 지우고, 토큰이 홀수 자리에
  오면 노출한다. 계약이 어느 인자가 비밀인지 말하게 해야 한다.
- **이름·학번만으로 신원을 주장하는 구조.** 사람이 "아직 모르겠다"고 했다 — 지금은 집계로
  보고, 위조가 이득이 되는 순간(회비·혜택과 이어질 때)이 오면 다시 본다.

### 검토가 바로잡은 숫자

제한 영역을 **31**로 적었는데 적어 둔 영역만 합치면 **28**이고, 빠진 `signedIn 3`을
넣어야 31이다. 영역은 19개 정의됐지만 **실제 쓰이는 것은 16개**다.
