...
- **요소 유형 레지스트리는 한 벌만 둔다.** 예전에는 플러그인의 UI와 code 번들이 각자 들고 있었고, ORG-02 작업에서 code 쪽 갱신이 누락돼 `note`·`group`·`list`가 있는 화면은 불러오기가 통째로 실패했다 - 그런데 테스트가 ui.mjs만 검사해 통과했다. **가드가 대상의 일부만 보면 없는 것과 같다.** 플러그인이 명세를 쓰지 않게 되면서 그 레지스트리와 검사는 사라졌지만, **규칙은 남았고 자리만 옮겼다**: 지금 요소 유형의 유일한 원본은 `screen.schema.json`의 `spec.type` enum이고, 검증 CLI가 그것으로 요소 스키마를 찾아 붙인다(`validate-specs.mjs`). 같은 계급이 2026-08-27 감사에서 다시 나왔다 - 요소를 훑는 곳이 다섯인데 넷이 최상위만 봤고, 그래서 중첩 요소가 스키마·판정·흐름·출처 추적에서 통째로 빠졌다. 지금은 `element-walk.mjs` 한 곳이 훑는다.
...
앞의 두 규칙은 검증 CLI가 요소 유형 전체를 훑어 강제하고(모르는 유형은 오류다), 마지막 규칙도 검증 CLI가 매번 검사한다. 플러그인이 명세를 쓰지 않게 된 경위는 `plugin-role.md`에 있다.

## 유형

- **유형의 목록을 여기 적지 않는다.** 유일한 원본은 `packages/contracts/schemas/screen.schema.json`의
  `spec.type` enum이고, 검증기 기동이 enum과 스키마 파일의 일치를 강제한다. 여기 베껴 두면
  뒤처진다 — 실제로 셋이라고 적힌 채 열이 될 때까지 그대로였고, 이 문서를 읽은 사람이
  없는 제약을 믿었다.
...
- `pending`은 **그림이 그렸지만 무엇인지 아직 정하지 않은 자리**다(2026-08-31). 버튼에는 오래전부터 `action.type: pending`이 있었지만 조작이 아니라 **칸 자체**가 그런 경우에 적을 자리가 없었고, 그런 칸은 명세에서 통째로 빠진 채 조용했다 — 목록 화면 넷이 검색칸 옆에 선택지도 문구도 없는 드롭다운 상자를 그렸는데(EVT-00A · OPS-MEET-01A 둘 · OPS-MEET-04B · REC-01) `select`로 적으려면 선택지 출처를 지어내야 하고 안 적으면 그림에 있는 칸이 명세에 없는 것이 된다. 세 번째 답이 **모른다는 것을 적는 것**이다. 값을 담지 않으므로 `fieldKey`가 없고 필수값 판정 후보에도 들어가지 않는다. `note`에는 '아직 미정'이 아니라 **그림이 무엇을 그렸고 어느 조각이 없어서 못 적는지**를 쓴다. 정해지면 이 요소를 제 종류로 바꾼다.
