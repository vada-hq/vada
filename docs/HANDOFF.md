...
- **REST로 직접 받는다(2026-08-24)**: 화면 하나의 산출물을 **플러그인을 열지 않고** 받는다. `node apps/spec-service/src/fetch-figma-screen.mjs vada-wireframe <screenId>` 한 줄이면 `figma.raw.json`·자산·`reference.png`에 `figma.design.json`까지 만들어진다.
  ...
  - **화면 목록을 볼 수 있다**: `node apps/spec-service/src/list-figma-screens.mjs vada-wireframe --todo`. 와이어프레임에 **화면 85개**가 있고 그중 13개가 명세됐다. 다음 화면을 제품 순서로 고르기로 했는데(implementation-methodology.md) 그 순서를 이제 Figma를 열지 않고 본다.
...
- **테스트**: spec-service·변환기·검증 91, vada-web(vitest) 172 + Playwright e2e 45 — 전부 통과. e2e는 AI가 직접 실행·스크린샷 판독하는 시각 검증 1차 수단이다(`apps/vada-web`에서 `npm run e2e`).
...
- **명세**: ONB-01·ONB-02·ORG-01·ORG-02·INV-01·HOME-01K·MY-01·OPS-00·TASK-01·OPS-MEET-01A·EVT-00A·EVT-TASK-01·EVT-TASK-02 13개 완결. 검증 오류 0건 경고 0건.
...
EVT-TASK-01(행사 업무 보드) 사이클을 마쳤다(`docs/pilot-evt-task-01.md`).
**칸반은 새 계급이 아니었다** — TASK-01이 이미 같은 모양이다. 여기서 처음인 것은
**인자가 오가는 것**이다. EVT-TASK-02는 받기만 했고, 그 값을 주는 쪽이 없어
주소로만 들어갔다.
...
1. **이동할 때 인자를 넘긴다**(새 개념) — `action.params`. 값의 출처가 셋에서
   넷이 됐고, 새로 생긴 `itemField`는 **명세도 화면도 모르는 값**이다(눌린 그 행만
   안다). 인자를 안 넘겨도 **이동은 성공하므로** 명세를 읽어서는 구멍이 안 보인다 —
   검증기가 셋을 본다(대상이 안 받는 인자, 아무도 안 주는 인자, 없는 조각).
2. **제목이 데이터다**(방아쇠) — EVT-TASK-02에서 미해결로 적어 둔 것의 두 번째
   발생이라 자리를 만들었다(`meta.titleFrom`). **`meta.title`이 겸하던 둘을 갈랐다** —
   화면의 이름과 그려지는 제목. INV-01의 `TITLE_NOT_DRAWN`이 그 긴장을 이미 적고 있었다.
3. **라벨 없이 값만 늘어놓는 묶음**(새 개념) — 값이 라벨까지 품고 온다
   (`담당 학술체육부 · 김바다`). 없는 라벨을 지어내면 디자인에 없는 카피가 되므로
   `summary.items[].label`을 선택으로 넓혔다.

### 이어 보고서야 드러난 것

보드에는 업무가 일곱인데 **와이어프레임이 상세로 그린 것은 하나뿐이다.** 나머지를
누르면 화면이 터졌다 — '개발용 응답이 없다'와 '찾지 못했다'가 섞여 있었다.
인자로 거른 결과가 빈 것은 **답이지 오류가 아니고**, 그 말은 이미 카탈로그에
있었다(`messages.empty`). 없는 여섯을 지어내지 않았다.

### 갈피 일곱은 옮겨 가는 것이다

행사 작업 공간의 갈피(개요·업무·문서·관련 회의·일정·인원 관리·재정)는 전부
와이어프레임의 **다른 화면**이다. 한 화면 안에서 내용이 바뀌는 갈피와 다르므로
`select`가 아니라 버튼 일곱이다. **이 머리는 행사 화면 일곱이 나눠 쓸 것**이라
두 번째 행사 화면을 만들 때가 셸로 뺄 때다.

**다음 후보**

1. **행사 개요**(`EVT-02`, `20:4671`) — 같은 작업 공간의 다음 갈피다. 행사 머리를
   두 번째로 쓰게 되므로 **셸로 뺄지**가 근거를 갖고 정해진다. EVT-00A의
   '행사 상세 보기' pending도 이것이 푼다.
