...
- **테스트**: spec-service·변환기·검증 99, vada-web(vitest) 220 + Playwright e2e 80 — 전부 통과. e2e는 AI가 직접 실행·스크린샷 판독하는 시각 검증 1차 수단이다(`apps/vada-web`에서 `npm run e2e`).
...
- **명세**: ONB-01·ONB-02·ORG-01·ORG-02·INV-01·HOME-01K·MY-01·OPS-00·TASK-01·OPS-MEET-01A·EVT-00A·EVT-02·EVT-TASK-01·EVT-TASK-02·EVT-DOC-01·EVT-MEET-01·EVT-SCHED-01·EVT-04 18개 완결. 검증 오류 0건 경고 0건.
...
EVT-04(행사 참가자 명단) 사이클을 마쳤다(`docs/pilot-evt-04.md`).
**행사 작업 공간의 여섯 번째 화면**이자 지금까지 중 가장 큰 화면이다(등록 12개,
두 번째로 많은 화면의 1.5배).

| 사이클 | 신규 계급 | 새 개념 | (A) 마찰 | 추출기 |
| --- | --- | --- | --- | --- |
| EVT-02 | 2 | 1 | 2 | — |
| EVT-DOC-01 | 1 | 1 | 0 | 2/3 |
| EVT-MEET-01 | 0 | 0 | 0 | 1/4 |
| EVT-SCHED-01 | 1 | 0 | 0 | 3/3 · 헛것 0 |
| **EVT-04** | **3** | **3** | **0** | **6/12 · 헛것 3** |

- **큰 화면은 아직 새 어휘를 만든다.** 앞의 세 사이클은 새 어휘 0이었는데 여기서
  셋이 났다 — 표현력이 수렴했다는 신호가 **작은 화면에서만** 관측된 것이었다.
  다만 셋 다 이미 있던 모양의 재사용으로 풀렸다(optionCounts·headerAction·
  workspace.source).
- **사람에게 물은 것은 셋뿐이다**(추출기 질문 24건 중). 나머지 21건은 대상 화면이
  아직 없거나 이름 짓기라 답이 정해져 있었다.
- **행사 작업 공간의 갈피 일곱 중 여섯이 찼다.** 남은 것은 재정 하나다.

**다음 후보**

1. **행사 작업 공간의 마지막 갈피**(`EVT-FIN-01` 재정) — 채우면 EVT-TASK-01의
   '아직 명세되지 않은 갈피' 검사가 없어진다. 재정은 뒤에 큰 계열(FIN-*)이 붙어
   있어 그 입구이기도 하다.
2. **추출기가 표를 못 본다** — 이제 사례가 둘이다(EVT-DOC-01·EVT-04). 열 머리 줄과
   되풀이 줄로 이루어진 같은 모양이라 규칙을 지어낼 근거가 생겼다.
