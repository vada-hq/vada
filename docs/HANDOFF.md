...
- **테스트**: 플러그인 89, spec-service·변환기·검증 82, vada-web(vitest) 150 + Playwright e2e 32 — 전부 통과. e2e는 AI가 직접 실행·스크린샷 판독하는 시각 검증 1차 수단이다(`apps/vada-web`에서 `npm run e2e`). 플러그인은 `manifest.json`을 Figma 데스크톱에서 불러온다.
...
EVT-00A(행사 목록) 사이클을 마쳤다(`docs/pilot-evt-00a.md`).
**신규 계급 1건, 스키마 확장 0건.**
...
### 판정에 셀 화면을 고르지 않기로 했다

미뤄 두었던 물음("계열 안에서 무엇이 같아야 연속인가")을 매듭지었다
(`docs/decisions/implementation-methodology.md`). **계열에는 예측력이 없다** —
OPS-MEET-01A의 요소 구성은 MY-01의 부분집합인데 계급이 셋 났다. 마찰은 요소 유형이
아니라 그 유형의 **속성**에서 나고, 속성은 화면을 열기 전에는 알 수 없다.
그래서 다음 화면은 **제품이 급한 순서**로 고르고, 연속은 시간순으로 센다.
판정은 목표가 아니라 관측이다.

### 이번 신규 계급 하나: 머리 오른쪽의 화면 동작

'완료된 행사 보기 →'는 헤더에 있지만 셸이 아니라 화면의 요소다. 명세는 `button`
그대로였고(`emphasis: quiet`), 없던 것은 **그릴 자리**였다 — `AppShell.headerAction`.

**그 자리가 없던 동안 조용히 빠진 것이 있었다.** TASK-01의 design `18:86`
'업무 추가'가 명세에도 화면에도 없었다. 대조는 **명세 → 화면 한 방향**이라 명세의
구멍은 통과한다. 이번에 메웠고, 반대 방향 검사를 BACKLOG에 올렸다.

### design 대조가 0곳에서 시작했다

MY-01 30곳 → 아홉 화면 234곳 → OPS-MEET-01A 6곳 → **EVT-00A 0곳**. 반증했다:
글자 색 한 곳을 틀리게 하니 9곳을 정확히 짚어 실패했다. 상태 딱지 규칙(-50/-200/-700)도
견뎌 `tones.ts`에 orange 한 줄만 늘었다.

### 즉시 처리 대상: 추출기

초안에 **버튼 4개만** 나왔다. 검색칸도 카드 목록도 머리의 이동도 없었다. 원인은
`screen-draft.mjs`가 **이름표 붙은 것만 보기** 때문이다 — `isFieldWrapper`는 직계
자식 `Label` 노드를, `readSection`은 묶음 제목을 요구한다. 앞 사이클의 오독과 원인은
다르지만 **같은 계급의 2사이클 연속 재발**이라 방법론상 즉시 처리 대상이다
(수렴 기준 (2)가 여기에 걸려 있다).

**다음 후보**

1. **추출기 고치기** — 위 재발. 화면당 사람 개입을 줄이는 기준 (2)의 병목이다.
2. **design에 있는데 명세에 없는 것 검사** — 반대 방향. TASK-01 누락이 근거다.
3. **업무 상세** — 제품에서 가장 급하다(MY-01·TASK-01·OPS-MEET-01A가 막혀 있다).
   `figma.raw.json`이 없어 사용자가 Figma에서 저장해야 시작된다.
