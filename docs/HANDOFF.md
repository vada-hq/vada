...
| 계약이 든 자리 | 219 |
| **서버가 답하는 자리** | **113** (`apps/api/src/app.test.ts`가 래칫으로 지킨다) |
| **화면이 부르는 자리 중 남은 것** | **99** (`node scripts/left.mjs`) |
| 화면이 읽는 출처 중 진짜 | **91 / 173** · 쓰기 **20 / 46** |
| 검사 | 계약 214 · 앱 1,074 · api 686 · 서버 통합 135 · e2e 428 · **카나리 59**(화면 57장을 배포 모양으로 걷는다) |
| 검증 | 오류 0건 · 경고 0건 |
| 배포 | **돈다** — `vada.vadahq-ops.workers.dev` · 구글 로그인까지 |
| 배포된 앱이 그리는 화면 | **49 / 74** — 나머지 25장은 `이 화면은 아직 준비 중입니다` |
