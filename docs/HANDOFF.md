...
- **명세**: 26개. **전부 구현됐다** — FIN-PROC-01이 2회차 교차 검증으로 붙으면서 비어 있던 마지막 하나가 찼다. 검증 오류 0건 경고 0건. **재정 갈래는 결제·증빙(FIN-EVID-01)만 남았다**: 재정 개요 → 내 구매 요청 → 요청 상세 → 보완 재제출, 재정부 쪽의 요청 검토와 구매·발주까지 이어져 있다.
...
**교차 검증 2회차가 끝났다.** 결과는 `docs/decisions/implementation-methodology.md`의
'2회차의 답'에 있다. 요약하면: 새 어휘 아홉이 **읽혔고**, 설명 하나가 이 저장소의
규칙을 스스로 어기고 있었다(`toneField`가 표현을 못박고 있었다). 그것을 고쳤고,
두 손이 각각 짚은 자리 하나를 스키마로 만들었다(`screen.activeNavigationScreenId`).

**다음은 `FIN-EVID-01`(결제·증빙 정리)이다.** 이것까지 만들면 FIN-REQ-02의 다섯
단계(요청 제출 · 재정부 검토 · 구매·발주 · 결제·증빙 · 처리 완료)가 전부 화면을
갖고, 재정 갈래가 처음부터 끝까지 이어진다. FIN-PROC-01의 마지막 버튼이 그것을
가리키며 pending으로 서 있다.

**그 뒤의 후보**

1. **재정 보드의 네 열이 어디로 가는지** — 지금은 넷 다 요청 상세로 간다.
   검토는 FIN-REV-01, 구매는 FIN-PROC-01로 가야 할 것 같은데 **기획 물음**이다.
2. **재정부가 보완 사유와 다시 받을 칸을 적는 자리가 없다** — FIN-REV-01에서
   품목을 보완으로 두면 FIN-SUP-01이 그것을 읽어야 하는데, 넣는 화면이 없다.
   **명세가 반쪽인 자리**이고 와이어프레임에 그림이 없다.
3. **추출기가 되풀이를 못 본다** — 사례가 셋이고 FIN-REQ-01이 헛것 39개를 냈다.
4. **회의 계열**(19개) — 가장 큰 덩어리인데 아직 한 화면만 봤다.
5. **3회차 교차 검증은 어휘가 또 늘면** — 1회차와 2회차 사이가 아홉이었다.
