...
- **명세**: ONB-01·ONB-02·ORG-01·ORG-02·INV-01·HOME-01K·MY-01·OPS-00 8개 완결. 검증 오류 0건 경고 0건.
...
OPS-00(운영 허브) 사이클을 마쳤다(`docs/pilot-ops00.md`). 신규 계급 3건인데
**셋 다 `summary` 하나를 넓힌 것이고 새 개념은 0건이다**(MY-01은 4건 중 1건이
새 개념이었다).
...
| INV-01 | 2 | 2 |
| HOME-01K | 3 | 3 |
| MY-01 | 4 | 1 (`shell.json`) |
| OPS-00 | 3 | **0** |

`shell.json`은 손대지 않고 그대로 쓰였다 — 한 번 세운 것이 재사용된 첫 사례다.

**그런데 이 화면은 수렴을 판정하지 못했다.** MY-01이 연 자리 셋(`itemList.params`
·`itemList.itemAction`·`select.optionCounts`)을 **하나도 쓰지 않았기 때문이다**.
OPS-00은 네비게이션 허브이고 MY-01은 작업 목록이라 표현 요구가 다르다.

이 사이클의 가장 중요한 발견은 이것이다. **"연속 2화면 신규 계급 0"의 '연속'을
데스크톱 화면 수로 세면 안 된다.** 같은 계열의 화면이라야 앞 화면이 연 자리가
다시 쓰이는지 볼 수 있다.

**다음 후보**

1. **MY-01과 같은 계열의 화면** — 목록·필터·탭이 있는 화면(운영의 하위인
   OPS-TASK-01 칸반 보드, 회의 목록, 행사 목록이 후보다). MY-01의 자리 셋이
   새 자리 없이 쓰이는지가 수렴의 첫 신호다.
2. **자산 소비 검사** — MY-01·OPS-00에서 두 번 연속 스크린샷 판독만이 아이콘
   결함을 잡았다. 준수 테스트가 자산을 보지 않는다. 트리거가 충족됐다.
