...
| --- | --- |
| Figma 프레임 | 85개 |
| 명세된 화면 | **85개**(변형 12 · 상태만 있는 것 4 포함) |
| 그리는 화면 | **81 / 81 = 100%**(요소를 가진 것) |
| 명세가 말하지 않는 글 | **0자** (`tests/screen-coverage.test.mjs`) |
| 계약이 든 자리 | 219 |
| **서버가 답하는 자리** | **158** (`apps/api/src/app.test.ts`가 래칫으로 지킨다) |
| **화면이 부르는 자리 중 남은 것** | **54** (`node scripts/left.mjs`) |
| 화면이 읽는 출처 중 진짜 | **133 / 173** · 쓰기 **23 / 46** |
| 검사 | 계약 214 · 앱 939 · api 891 · 서버 통합 168 · e2e 428 · **카나리 59**(화면 57장을 배포 모양으로 걷는다) |
| 검증 | 오류 0건 · 경고 0건 |
| 배포 | **돈다** — `vada.vadahq-ops.workers.dev` · 구글 로그인까지 |
| 배포된 앱이 그리는 화면 | **59 / 74** — 나머지 15장은 `이 화면은 아직 준비 중입니다`(재정 6 · 메시지 3 · 결정 대기 셋 · 자리 단위로 가린 셋) |
...
```
밖에서    0    ✅        회의    9
들어오기  0    ✅        기록    5   ← 승인 단추(발행)를 그려야 나머지가 연다
조직      2              재정   28   ← 예산 화면을 그려야 연다
                         행사   10
```
...
**3회차는 뼈대부터 갔다**(2026-09-05). 자리 서른을 골라 붙여 화면 열아홉 장이
한꺼번에 '준비 중'을 벗어났다 — 자리 하나나 둘만 모자란 화면을 먼저 골랐다.
남은 자리가 아니라 **선 화면**이 사람이 보는 것이다.
