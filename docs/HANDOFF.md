...
- **명세**: ONB-01·ONB-02·ORG-01·ORG-02·INV-01·HOME-01K·MY-01·OPS-00·TASK-01·OPS-MEET-01A·EVT-00A·EVT-02·EVT-TASK-01·EVT-TASK-02·EVT-DOC-01·EVT-MEET-01·EVT-SCHED-01·EVT-04·EVT-FIN-01·FIN-REQ-01 20개 완결. 검증 오류 0건 경고 0건. **행사 작업 공간의 갈피 일곱이 전부 찼고, 그 아래 재정 계열의 첫 화면이 붙었다.**
...
FIN-REQ-01(구매 요청 작성·수정) 사이클을 마쳤다(`docs/pilot-fin-req-01.md`).
**행사 화면 일곱을 지나 처음으로 값을 쓰고 보내는 화면**이고, 등록 27개로 가장 크다.
...
| **FIN-REQ-01** | **27** | **5** | **6** | **0** | **18/27 · 헛것 41** |

- **작은 화면에서만 0건이 났다는 관찰이 다시 확인됐다.** 등록 3~4개짜리는 0~1건,
  9~12개짜리는 1~3건, 27개짜리는 5건이다. 수렴 판정의 조건 (1)은 **화면 크기와 함께
  읽어야 한다** — 크기를 적지 않은 0건은 근거가 되지 못한다.
...
1. **재정 계열을 이어 내려간다** — `FIN-REQ-02`(구매 요청 상세·진행 상태) 또는
   `FIN-REV-01`(구매 요청 검토). EVT-FIN-01의 카드를 여는 자리가 아직 `pending`이고,
   방금 만든 어휘(`itemFields`·`compute`·`draftFrom`)가 **두 번째 화면에서도 맞는지**가
   그 자리에서 판가름 난다. 한 번 쓰인 어휘는 아직 어휘가 아니다.
2. **추출기가 되풀이를 못 본다** — 사례가 셋이 됐고(EVT-DOC-01·EVT-04·FIN-REQ-01)
   마지막 것이 헛것 39개를 냈다. 눈금 전체 헛것의 대부분이 이 한 계급이다.
