...
| 검사 | 계약 170 · 앱 1440 · e2e 425(+1 skip) |
| 검증 | 오류 0건 · 경고 3건 |
...
- **데이터 출처의 문구 435개 중 290개를 아무 화면도 읽지 않는다** — `loading` 0곳 ·
  `error` 2곳. 읽기가 전부 동기라 기다리는 시간도 실패할 통신도 없다. 명세는 옳고
  참조 구현이 명세의 3분의 1을 증명하지 않는다.
