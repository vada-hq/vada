...
- **테스트**: spec-service·변환기·검증 99, vada-web(vitest) 229 + Playwright e2e 86 — 전부 통과. e2e는 AI가 직접 실행·스크린샷 판독하는 시각 검증 1차 수단이다(`apps/vada-web`에서 `npm run e2e`).
...
- **명세**: ONB-01·ONB-02·ORG-01·ORG-02·INV-01·HOME-01K·MY-01·OPS-00·TASK-01·OPS-MEET-01A·EVT-00A·EVT-02·EVT-TASK-01·EVT-TASK-02·EVT-DOC-01·EVT-MEET-01·EVT-SCHED-01·EVT-04·EVT-FIN-01 19개 완결. 검증 오류 0건 경고 0건. **행사 작업 공간의 갈피 일곱이 전부 찼다.**
...
EVT-FIN-01(행사 재정 — 개요) 사이클을 마쳤다(`docs/pilot-evt-fin-01.md`).
**행사 작업 공간의 일곱 번째이자 마지막 갈피**다 — 이 공간은 이제 다 찼다.

| 사이클 | 등록 | 신규 계급 | 새 개념 | (A) 마찰 | 추출기 |
| --- | --- | --- | --- | --- | --- |
| EVT-DOC-01 | 3 | 1 | 1 | 0 | 2/3 |
| EVT-MEET-01 | 4 | 0 | 0 | 0 | 1/4 |
| EVT-SCHED-01 | 3 | 1 | 0 | 0 | 3/3 · 헛것 0 |
| EVT-04 | 12 | 3 | 3 | 0 | 6/12 · 헛것 3 |
| **EVT-FIN-01** | **9** | **1** | **1** | **0** | **3/9 · 헛것 2** |

- **행사 계열이 한 덩이로 완성됐다.** 화면 일곱이 같은 머리를 나눠 쓰고, 갈피
  일곱이 전부 이어진다. e2e가 그 왕복을 통째로 한 번에 본다.
- **마지막 계급은 도구가 못 잡는 자리에서 났다** — 화면 제목은 셸이 그리므로
  design 대조 밖이다. 검사는 다 통과했고 사람이 e2e를 쓰다가 봤다.

**다음 후보**

1. **재정 계열로 내려간다**(`FIN-REQ-01` 구매 요청 작성·수정 → `FIN-PROC-01` →
   `FIN-EVID-01`) — EVT-FIN-01이 그 넷의 입구이고 지금 카드마다 `pending`이다.
   **작성·제출이 있는 화면**이라 지금까지의 읽기 전용 화면들과 성격이 다르다.
2. **추출기가 표를 못 본다** — 사례가 둘이다(EVT-DOC-01·EVT-04). EVT-04는 겉 이름이
   `Table`, 줄이 `Table Row`라 이름까지 신호를 준다. 트리거는 이미 찼다.
3. **게이트 실행을 쪼갠다** — 벽시계가 예산(60초) 언저리에서 흔들린다.
