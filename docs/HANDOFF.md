...
| 계약이 든 자리 | **217** (선택지 다섯이 명세에 박히며 222에서 줄었다 — 구매 셋, 회의 둘) |
...
| 검사 | 계약 217 · 앱 951 · api 1,162 · 서버 통합 218 · e2e 428 · **카나리 60**(화면 58장을 배포 모양으로 걷는다 — **터질 것으로 둔 화면이 이제 없다**) · **연기**(배포된 사슬 8곳) |
| 검증 | 오류 0건 · 경고 0건 |
| 배포 | **돈다** — `vada.vadahq-ops.workers.dev` · 구글 로그인까지 |
| 배포된 앱이 그리는 화면 | **71 / 75** — 나머지 4장은 `이 화면은 아직 준비 중입니다`(메시지 3 · 아카이브 체크리스트 1) |
| 카나리가 아직 못 재는 것 | 신원 하나(권한이 다른 사람) · 로그인 전 화면 · 밖에서 오는 사람 · **로그인한 뒤의 배포된 화면**(연기 검사도 못 본다 — 구글 열쇠가 저장소에 없다) |

### 영역별로 얼마나 남았나

```
밖에서    0    ✅        회의    0   ✅  진행 방식·소요 시간을 명세가 들었다
...
**뼈대부터 갔다**(2026-09-05~06). 자리 하나나 둘만 모자란 화면부터 세웠더니 이틀에 준비 중이 40장에서 넷이 됐다. 남은 넷은 사람이 뺐거나 미룬 것이다 — 메시지 셋, 아카이브 체크리스트 하나. 다음은 살이다: 백로그 '지금'의 스물 남짓과 결정 대기 셋.
