...
- **REST로 직접 받는다(2026-08-24)**: 화면 하나의 산출물을 **플러그인을 열지 않고** 받는다. `node apps/spec-service/src/fetch-figma-screen.mjs vada-wireframe <screenId>` 한 줄이면 `figma.raw.json`·자산·`reference.png`에 `figma.design.json`까지 만들어진다.
  ...
  - **화면 목록을 볼 수 있다**: `node apps/spec-service/src/list-figma-screens.mjs vada-wireframe --todo`. 와이어프레임에 **화면 85개**가 있고 그중 28개가 명세됐다. 다음 화면을 제품 순서로 고르기로 했는데(implementation-methodology.md) 그 순서를 이제 Figma를 열지 않고 본다.
...
- **테스트**: 계약 127, vada-web(vitest) 431 + Playwright e2e 135 — 전부 통과. 게이트는 두 앱을 **함께** 돌리고, 대조 검사가 SVG 391개를 번들러 대신 디스크에서 읽는다(벽시계 34~50초). e2e는 AI가 직접 실행·스크린샷 판독하는 시각 검증 1차 수단이다(`apps/vada-web`에서 `npm run e2e`).
...
ORG-03A(조직도)까지 왔다. 어휘가 둘 늘었다 — `itemList.emptyAction`(부서장이
없는 부서에만 지정 버튼이 그려진다)과 `itemList.titleField`('부원 2명'이 글자
하나다).

**다음은 ORG-03B(조직 관리 — 수정)다.** 같은 나무를 고치는 화면이고, 그 뒤가
**ORG-03C(구성원 초대 패널 — 첫 모달)**다. 모달은 아직 한 번도 안 봤다:
