...
- **REST로 직접 받는다(2026-08-24)**: 화면 하나의 산출물을 **플러그인을 열지 않고** 받는다. `node apps/spec-service/src/fetch-figma-screen.mjs vada-wireframe <screenId>` 한 줄이면 `figma.raw.json`·자산·`reference.png`에 `figma.design.json`까지 만들어진다.
  ...
  - **화면 목록을 볼 수 있다**: `node apps/spec-service/src/list-figma-screens.mjs vada-wireframe --todo`. 와이어프레임에 **화면 85개**가 있고 그중 34개가 명세됐다. 다음 화면을 제품 순서로 고르기로 했는데(implementation-methodology.md) 그 순서를 이제 Figma를 열지 않고 본다.
...
**모달을 봤다.** ORG-07B가 그것이고, 어휘 `screen.overlay`가 나왔다 — 어느 화면
위에 뜨는가와 이 화면이 그리는 부분이 디자인의 어디인가. 그 둘을 말하지 않으면
화면을 만드는 사람이 뒤를 지어내고, 검증기가 아래 화면의 버튼을 '명세에 없는
상호작용'으로 본다.

**다음은 ORG-07C(학생회비 납부 명단 업로드 모달)다.** 조직 갈래의 마지막 화면이고,
`overlay`가 두 번째로 쓰이는 자리다 — 한 번 쓴 어휘가 다음 화면에서 그대로 서는지
보는 것이 이 사이클의 값이다.
...
2. **조직 관리(9개)** — ORG-01·ORG-02는 이미 있고 ORG-00·03·04·07 계열이 남았다.
   모달이 셋 있는데(`ORG-03C`·`ORG-07B`·`ORG-07C`) **모달은 아직 한 번도 안 봤다.**
3. **기록(3개)** — REC-01·02·02A. 완료된 행사의 아카이브다.
4. **메시지(3개)·외부 참여(5개)** — 지금까지와 성격이 가장 다르다.
   외부 참여는 로그인하지 않은 사람이 보는 화면이라 **셸이 없을 수 있다.**

**모달이 있는 조직 관리를 권한다.** 지금까지 화면은 전부 한 자리를 통째로 쓰는
것이었고, 모달은 그렇지 않다 — 어느 화면 위에 뜨는지, 닫으면 어디로 가는지가
새 물음이다. 그리고 그 물음은 재정 갈래를 더 파도 나오지 않는다.
