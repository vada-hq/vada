...
| 계약이 든 자리 | **219** (선택지 다섯이 명세로 가며 222→217, 내보내기 둘이 붙어 219) |
| **서버가 답하는 자리** | **209** (`apps/api/src/app.test.ts`가 래칫으로 지킨다) |
| **화면이 부르는 자리 중 남은 것** | **5** (`node scripts/left.mjs`) — 메시지 4 · 아카이브 체크리스트 1 · 그 밖 0 |
| 화면이 읽는 출처 중 진짜 | **163 / 172** · 쓰기 **42 / 47** |
| 검사 | 계약 217 · 앱 960 · api 1,188 · 서버 통합 219 · e2e 429 · **카나리 61**(화면 59장을 배포 모양으로 걷는다 — **터질 것으로 둔 화면이 없다**) · **연기**(배포된 사슬 8곳) |
| 검증 | 오류 0건 · 경고 0건 |
