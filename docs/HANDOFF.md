...
다음은 **요소 등록 자동 추출기**다. 등록 노드 계약이 생겨 기계적 추출의 정답이 하나로 정해졌고, 실측에서 요소·유형·라벨·필수는 이미 7/7이었다(정답지 비공개 상태로 작성한 추출기, ONB-01 기준. 계층 7단계도 문제 없었다). 남는 것은 사람만 아는 값 — fieldKey 작명, 드롭다운 선택지 내용, note의 의도, 애매한 유형 판별 — 이므로, **AI가 초안을 뽑고 사람은 그 3~4줄만 답하는 형태**로 다음 화면 사이클에서 시험한다.

이와 별개로 화면 사이클 4~7단계(저장·AI 인계·동기화·최종 확인) 검토가 남아 있다.
