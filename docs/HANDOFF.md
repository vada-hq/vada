...
| 검사 | 계약 169 · 앱 1353 · e2e 425(+1 skip) |
| 검증 | 오류 0건 · 경고 3건 |
...
- **명세 안의 `pending`이 73곳** — "아직 정하지 않았다"로 남은 자리다. 남은 제품
  작업의 크기는 백로그 줄 수가 아니라 이 숫자가 말한다.
...
- **명세와 화면을 짝지어 보는 가장 센 검사가 정규식이다**(`screen-honors-spec.test.ts`).
  그 파일이 스스로 "있는데 틀리게 쓴 것은 못 잡는다"고 적어 두었다.
