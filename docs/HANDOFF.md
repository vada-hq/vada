...
- **눈금이 화면 전부를 본다(2026-08-25)**: `screen-draft.test.mjs`의 래칫이 화면 열한 개를 **손으로 적어** 두고 있었다. 뒤에 만든 작업 공간 화면 다섯이 눈금 밖이라, 그 화면들에서 헛것이 28개까지 자라는 동안 검사는 조용했다. 이제 `screen.json`이 있는 화면을 전부 센다 — 화면을 하나 더 명세하면 저절로 눈금에 든다. **등록 100개 중 맞춤 49(48→49) · 헛것 26(59→26).**
