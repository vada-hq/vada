...
| --- | --- |
| Figma 프레임 | 85개 |
| 명세된 화면 | **87개**(변형 12 · 상태만 있는 것 4 포함) — 2026-09-06 ORG-03D 구성원 내보내기 확인을 그렸다 |
| 그리는 화면 | **82 / 82 = 100%**(요소를 가진 것) |
| 명세가 말하지 않는 글 | **0자** (`tests/screen-coverage.test.mjs`) |
| 계약이 든 자리 | **220** (선택지 다섯이 명세로 가며 222→217, 내보내기 둘과 참여 하나가 붙어 220) |
| **서버가 답하는 자리** | **210** (`apps/api/src/app.test.ts`가 래칫으로 지킨다) |
| **화면이 부르는 자리 중 남은 것** | **5** (`node scripts/left.mjs`) — 메시지 4 · 아카이브 체크리스트 1 · 그 밖 0 |
| 화면이 읽는 출처 중 진짜 | **163 / 172** · 쓰기 **42 / 47** |
| 검사 | 계약 218 · 앱 974 · api 1,214 · 서버 통합 223 · e2e 429 · **카나리 61**(화면 59장을 배포 모양으로 걷는다 — **터질 것으로 둔 화면이 없다**) · **연기**(배포된 사슬 9곳) |
| 검증 | 오류 0건 · 경고 0건 |
| 그림 대조의 자리 예외 | **87** (2026-09-06에 102에서 줄었다 — 초안이 값 곁에 이름을 실어 오게 해 열다섯이 사라졌다) |
| 배포 | **돈다** — `vada.vadahq-ops.workers.dev` · 구글 로그인까지 |
| 배포된 앱이 그리는 화면 | **72 / 76** — 나머지 4장은 `이 화면은 아직 준비 중입니다`(메시지 3 · 아카이브 체크리스트 1) |
| 카나리가 아직 못 재는 것 | 신원 하나(권한이 다른 사람) · 로그인 전 화면 · 밖에서 오는 사람 · **로그인한 뒤의 배포된 화면**(연기 검사도 못 본다 — 구글 열쇠가 저장소에 없다) |
...
들어오기  0    ✅        조직    0   ✅
행사      0    ✅        재정    0   ✅
                         기록    5   ← 메시지 넷(뺌) · 체크리스트(멈춤)
```

**남은 것은 기록 하나뿐이다.** 그 다섯도 사람이 뺐거나(메시지 넷) 멈춰 둔 것(아카이브
체크리스트)이라, 지금 지을 수 있는 자리는 없다.

**뼈대부터 갔다**(2026-09-05~06). 자리 하나나 둘만 모자란 화면부터 세웠더니 이틀에
준비 중이 40장에서 넷이 됐다. 남은 넷은 사람이 뺐거나 미룬 것이다 — 메시지 셋,
아카이브 체크리스트 하나. **다음은 살이다**: 백로그 '지금'의 스물 남짓.
