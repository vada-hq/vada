...
- **명세**: 26개(위 19개 + FIN-REQ-01·FIN-REQ-02·MY-REQ-01·FIN-SUP-01·FIN-REV-01·FIN-PROC-01·EVT-FIN-01). **FIN-PROC-01만 구현이 없다 — 2회차 교차 검증을 위해 일부러 비워 뒀다.** 검증 오류 0건 경고 0건. **재정 갈래가 한 바퀴 돌았다**: 재정 개요 → 내 구매 요청 → 요청 상세 → 보완 재제출, 그리고 재정부 쪽의 요청 검토. 남은 것은 구매·발주(FIN-PROC-01, 명세만)와 결제·증빙(FIN-EVID-01, 아직 없음)이다.
...
**2회차 교차 검증이 걸려 있다.** `FIN-PROC-01`(구매·발주 처리)의 **명세만 쓰고
구현은 일부러 비워 뒀다.** 지시문은 `docs/cross-check-fin-proc-01.md`.

1회차(FIN-REQ-02)가 물은 것은 **명세만으로 화면을 만들 수 있는가**였다. 답은
"만들 수는 있는데 일곱 군데를 추측했다"였고, 그 일곱 중 넷이 스키마가 됐다.
거기서 알게 된 것이 이것이다 — **명세가 가리키지 않는 조각은 검증기가 검사할 수
없다.** 스무 화면이 `row.statusTone`을 코드에 박아 쓰고 있었는데 아무도 몰랐다.

**2회차가 묻는 것은 다르다: 그 뒤로 늘어난 어휘가 읽히는가.** 1회차 이후 스키마가
아홉 군데 늘었고(breadcrumb · missingNote · columns[].toneField · columns[].fieldKey ·
itemList.itemFields · itemList.fieldKey · fieldSet · button.labelHidden/
labelWhenAnyItemIs · group.headerFields) **전부 한 손에서 나왔다.** 지은 사람은 늘
자기 어휘를 읽을 수 있고, 그것은 아무것도 말해 주지 않는다.

- **보여줄 것**: `specs/figma/vada-wireframe/` · `packages/contracts/` ·
  `apps/vada-web/src/components/`·`design/`·`spec/` · `docs/decisions/`
- **가릴 것**: `apps/vada-web/src/screens/`(이미 만든 화면 스물셋)
- **채점**: 사람이 아니라 이미 있는 게이트가 한다
- **산출물**: 통과/실패보다 **막힌 자리**, 그리고 **읽기 어려웠던 이름**

FIN-PROC-01에서 처음인 것은 `group.headerFields`다. `itemList.group` 자체는
OPS-MEET-01A에 선례가 있지만 거기에는 `columns`가 없어서, **묶음과 열이 함께
오는 것은 이번이 처음이다** — 둘은 반대 방향을 가리킨다(열은 묶음 **안**, 머리는
묶음 **자신**). 추출기는 3개 중 2개를 맞히고 **헛것이 0**이었다.

**교차 검증이 끝난 뒤의 후보**

1. **막힌 자리와 나쁜 이름을 명세에 반영한다** — 이 실험의 본래 목적이다.
2. **`FIN-EVID-01`(결제·증빙)** — 이것까지 만들면 FIN-REQ-02의 다섯 단계가 전부
   화면을 갖는다.
3. **재정 보드의 네 열이 어디로 가는지** — 지금은 넷 다 요청 상세로 간다.
   검토는 FIN-REV-01, 구매는 FIN-PROC-01로 가야 할 것 같은데 **기획 물음**이다.
4. **추출기가 되풀이를 못 본다** — 사례가 셋이고 FIN-REQ-01이 헛것 39개를 냈다.
5. **회의 계열**(19개) — 가장 큰 덩어리인데 아직 한 화면만 봤다.
