...
- **고른 것은 색으로 드러난다(2026-08-25)**: 나란한 버튼이 '각각 별도 버튼'인지 '하나 고르기'인지를 이제 기계가 답한다 — **골라진 하나만 배경이 다르고 나머지가 서로 같으면** choiceGroup이다. 이 wireframe의 나란한 버튼 묶음 스물아홉 중 셋이 걸리고 전부 실제로 choiceGroup이다(오검출 0). 행동 짝(`이전`/`조직 만들기`)이 걸리지 않는 이유가 핵심이다 — **둘일 때는 '하나만 다르다'가 성립하지 않는다.** 둘은 서로 다르거나 서로 같거나이지 하나만 튀는 상태가 없다. 그래서 개수 조건을 따로 두지 않는다. 조건을 '나머지와 다른 것이 있으면'으로 느슨하게 하면 검사 넷이 깨지는 것을 확인했다. **EVT-DOC-01의 질문이 20건 → 2건이 됐다.**
...
- **눈금이 화면 전부를 본다(2026-08-25)**: `screen-draft.test.mjs`의 래칫이 화면 열한 개를 **손으로 적어** 두고 있었다. 뒤에 만든 작업 공간 화면 다섯이 눈금 밖이라, 그 화면들에서 헛것이 28개까지 자라는 동안 검사는 조용했다. 이제 `screen.json`이 있는 화면을 전부 센다 — 화면을 하나 더 명세하면 저절로 눈금에 든다. **등록 100개 중 맞춤 51(48→51) · 헛것 17(59→17).**
