...
- **명세**: 위 19개 + FIN-REQ-01·**FIN-REQ-02** 21개. **FIN-REQ-02만 구현이 없다 — 교차 검증을 위해 일부러 비워 뒀다.** 검증 오류 0건 경고 0건. **행사 작업 공간의 갈피 일곱이 전부 찼고, 그 아래 재정 계열의 첫 화면이 붙었다.**
...
**교차 검증이 걸려 있다.** `FIN-REQ-02`(구매 요청 상세·진행 상태)의 **명세만 쓰고
구현은 일부러 비워 뒀다.** 지시문은 `docs/cross-check-fin-req-02.md`.

왜 하는가. 이 저장소의 주장은 **"명세 하나만 보면 화면을 만들 수 있다"**인데,
지금까지 명세를 쓴 쪽과 구현한 쪽이 같았다. 명세에 안 적고 머릿속에만 갖고 있던
것이 있어도 구현할 때 그냥 써버리니 아무도 모르고, **게이트도 못 잡는다** — 명세와
화면을 견주는데 둘 다 같은 손에서 나왔기 때문이다. 검사를 더 만들어서는 못 푼다.

지금이 적기인 이유는 **바로 앞 사이클에서 어휘가 다섯 개 늘었기** 때문이다.
`itemFields`·`compute`·`draftFrom`은 한 번씩만 쓰였고, 한 번 쓰인 어휘는 아직
어휘가 아니다. 머릿속 뜻과 적힌 뜻이 갈라져 있을 확률이 가장 높은 시점이다.

- **보여줄 것**: `specs/figma/vada-wireframe/` · `packages/contracts/` ·
  `apps/vada-web/src/components/`·`design/`·`spec/` · `docs/decisions/`
- **가릴 것**: `apps/vada-web/src/screens/`(이미 만든 화면 스무 개)
- **채점**: 사람이 아니라 이미 있는 게이트가 한다
- **산출물**: 통과/실패보다 **막힌 자리**다

FIN-REQ-02 명세에서 새로 생긴 어휘 — `steps`(정해진 단계와 '지금 어디인지'.
단계는 명세가 알고 지금은 데이터가 안다), `summary.eyebrowField`, `summary.status`.
추출기는 5개 중 3개를 맞히고 **헛것이 0**이었다(단계 줄과 요약 카드는 어휘가 없어
지어내지 않고 지나갔다).

**교차 검증이 끝난 뒤의 후보**

1. **막힌 자리를 명세에 반영한다** — 이 실험의 본래 목적이다.
2. **재정 계열을 이어 내려간다** — `FIN-REV-01`(검토) · `FIN-SUP-01`(보완 재제출) ·
   `FIN-PROC-01`(구매·발주) · `FIN-EVID-01`(결제·증빙).
3. **추출기가 되풀이를 못 본다** — 사례가 셋이 됐고(EVT-DOC-01·EVT-04·FIN-REQ-01)
   마지막 것이 헛것 39개를 냈다.
4. **회의 계열**(19개) — 가장 큰 덩어리인데 아직 한 화면만 봤다.
