...
- **REST로 직접 받는다(2026-08-24)**: 화면 하나의 산출물을 **플러그인을 열지 않고** 받는다. `node apps/spec-service/src/fetch-figma-screen.mjs vada-wireframe <screenId>` 한 줄이면 `figma.raw.json`·자산·`reference.png`에 `figma.design.json`까지 만들어진다.
  ...
  - **화면 목록을 볼 수 있다**: `node apps/spec-service/src/list-figma-screens.mjs vada-wireframe --todo`. 와이어프레임에 **화면 85개**가 있고 그중 38개가 명세됐다(그중 셋은 변형이다). 다음 화면을 제품 순서로 고르기로 했는데(implementation-methodology.md) 그 순서를 이제 Figma를 열지 않고 본다.
...
- **명세**: 27개, 전부 구현됐다. 검증 오류 0건 경고 0건. **재정 갈래가 처음부터 끝까지 이어졌다** — 재정 개요 → 내 구매 요청 → 요청 작성 → 요청 상세 → 보완 재제출, 재정부 쪽으로 요청 검토 → 구매·발주 → 결제·증빙. FIN-REQ-02가 그리는 다섯 단계가 전부 화면을 갖는다.
...
- **제목이 공간의 것이 아닌 화면(2026-08-25, `workspace.ownTitle`)**: 작업 공간에 속하면 제목은 그 공간의 이름이다 — 행사 개요도 행사 업무도 제목이 그 행사의 이름이다. **EVT-FIN-01만 자기 이름을 쓴다**(`행사 재정 — 개요`). 벗어날 길이 `meta.titleFrom`(다른 데이터에서 제목을 읽는다) 하나뿐이었는데 여기 제목은 고정 문구라 그 길로 갈 수 없다. 뜻은 "이 화면은 공간의 제목 대신 자기 이름을 쓴다"이고, 왜 그러는지도 함께 적었다 — **자기 아래에 다시 여러 화면을 거느리는 입구**이기 때문이다(재정은 구매 요청·처리·증빙·정산 넷의 입구다). **이 자리가 없으면 아무도 못 잡는다**: 화면 제목은 셸이 그리므로 등록 노드 밖이고 design 대조는 안쪽만 본다. 검사는 전부 통과했고 e2e를 쓰다가 드러났다.
- **스펙 체계 확장(2026-08-25, EVT-04 사이클)**: 가장 큰 화면(등록 12개)에서 새 어휘 셋이 났다. (1) **`itemList.paging`** — 목록이 쪽으로 나뉘면 총 몇 건인지·몇 쪽인지를 자기가 말할 수 없다(한 쪽만큼만 받아 온다). 그것을 아는 출처를 따로 가리키는 자리이고 `select.optionCounts`와 같은 모양이다. 쪽 번호는 화면 상태지만 **필드가 아니라서**(담는 컨트롤이 없다) `params`에 적지 않고 `pageParam`이 이름만 말한다. (2) **`itemList.selection`** — 항목을 여럿 고르는 표. `action`이 필수다: 고른 다음 무엇을 하는지가 없으면 고르기도 없다. (3) **작업 공간 상태 줄의 행동** — 스키마는 한 줄도 안 늘었다. 다른 다섯 화면이 그 자리에 그리는 `행사 관리 행동은 담당 운영진에게 제공됩니다`가 **이 자리의 임자가 누구인지 이미 말하고 있었고**, 안내는 '행동이 없다'는 말이므로 행동이 있으면 안내가 설 자리가 없다(`WorkspaceHeader.actions`, `AppShell.headerAction`과 같은 규칙).
- **한 개념이 두 자리에 그려진다(2026-08-25)**: 등록 노드 계약은 "요소의 모든 부분을 포함하는 가장 안쪽 노드"인데, EVT-04의 디자인은 **표와 쪽 줄을 형제로 둔다** — 둘을 함께 품는 노드가 없다. 작업 공간이 이미 같은 모양이라(`workspace.source`가 갈피 줄·상태 줄의 자리를 따로 갖는다) `paging.source`가 쪽 줄의 자리를 갖고, 등록 노드를 모으는 **두 곳**(검증기의 역방향 검사, design 대조의 `registeredNodeIds`)이 함께 읽는다. 이 자리가 없었다면 쪽 버튼 셋이 '명세에 없는 상호작용 노드'로 걸렸을 것이고, 실제로 그 검사가 먼저 알려 줬다.
- **조회 인자의 모양이 하나가 됐다(2026-08-25)**: `select.optionsSource.params`만 `{인자: fieldKey}`라는 옛 모양이었고, 그래서 **화면이 밖에서 받은 값(screenParam)으로 선택지를 좁힐 수 없었다** — EVT-04의 거르기 넷이 '어느 행사의 참가자인지'를 넘겨야 해서 드러났다. 공용 모양(`element-params.schema.json`)으로 바꾸고 기존 매핑 다섯을 옮겼다(ONB-01 3 · INV-01 3 · ORG-01 1). 검증기의 판정도 `checkArgumentValues` 한 곳으로 모았다 — 인자의 출처가 늘면 이제 세 자리가 함께 는다.
- **행사 작업 공간이 다 찼다(2026-08-25, EVT-FIN-01)**: 갈피 일곱이 전부 화면을 가리킨다(개요·업무·문서·관련 회의·일정·인원 관리·재정). 마지막 화면에 새 어휘는 하나뿐이었고(위의 `ownTitle`) 나머지는 전부 이미 있던 것이다 — 금액 타일은 EVT-TASK-01의 요약, 처리 단계 열 넷은 EVT-TASK-01의 칸반, 화면 안 갈피는 EVT-04, 빈 열의 '항목 없음'은 데이터 출처의 `messages.empty`다. **금액은 화면이 계산하지 않는다** — 배정에서 무엇을 빼야 사용 가능액이 되는지가 곧 조직의 재정 규칙이라 네 값이 다 따로 온다. 사이클마다 옮겨 다니던 '아직 명세되지 않은 갈피' e2e는 옮겨 갈 빈 갈피가 없어져 지웠다.
- **추출기가 화면 하나를 통째로 맞혔다(2026-08-25, EVT-SCHED-01)**: 작업 공간의 다섯 번째 화면이다. 등록 3개를 자리·유형·라벨까지 다 맞히고 없는 것을 하나도 뽑지 않았다(3/3·헛것 0) — **처음이다.** 앞 사이클에서 고친 규칙(작업 공간 갈피 줄·고르기 묶음·되풀이 판별)이 미리 보고 맞춘 적 없는 화면에서 그대로 통했다. 남은 질문 4건은 전부 디자인 밖의 것이다(이동 대상·이름 짓기·개수의 출처). **조건 (2)는 여전히 아니지만 질문의 성격이 달라졌다** — 예전에는 "이것이 무엇이냐"였고 지금은 "뭐라 부를 거냐 / 어디로 갈 거냐"다. 이 화면도 `packages/contracts`를 한 줄도 손대지 않았다. 요점은 **이 목록이 원본이 아니라는 것**이다 — 업무·회의·행사 기본정보가 각자 원본이고 줄마다 `originNote`가 어디를 고치는지 말한다. 그래서 줄을 누르면 **줄마다 다른 화면**으로 가야 하는데 지금 어휘에 없어(`itemAction`은 대상이 하나) `pending`으로 남겼다.
- **새 어휘 없이 화면이 나왔다(2026-08-25, EVT-MEET-01)**: 작업 공간의 네 번째 화면인데 `packages/contracts`를 한 줄도 손대지 않았다. **신규 마찰 계급 0건이 연속 둘**이라 수렴 판정의 조건 (1)이 채워졌다 — 조건 (2)(사람 개입 고정)는 아직 아니다(추출기가 요소 4개 중 1개만 뽑는다). 여기서 처음인 것은 **공간 밖의 이미 있는 화면으로 나가는 이동**이다(`전체 회의 보기` → OPS-MEET-01A). 그 화면은 행사에 매이지 않아 인자를 받지 않으므로 **아무것도 넘기지 않는 것이 맞다** — EVT-TASK-01에서 만든 인자 검사가 여기서 처음 그렇게 답했다. 안내 배너는 EVT-02의 것과 같은 요소이고 값의 출처만 다르다(`items[].field` → `items[].value`).
...
- **design 대조(2026-08-23~24)**: `figma.design.json`과 실제로 그려진 DOM을 게이트에서 견준다(`src/design-check/`). 화면이 `data-node-id`로 등록 노드를 지목하면 그 안의 **글·글자 색·굵기·칸의 배경/테두리색·그림**을 대조한다. 붙일 때 234곳이 어긋나 있었다.
  ...
  - **못 보는 것**: 한 방향이다(화면에만 있는 장식은 못 잡는다), 등록 노드 밖(셸)은 안 본다, 같은 글이 여럿이면 하나만 맞아도 통과, 여백·크기·위치·글자 크기는 안 본다(와이어프레임의 루트 글꼴이 14px이라 rem에서 온 값이 전부 0.875배로 나오고, 그 환산이 잡음을 섞는다 — interpretation.md).
...
- **마찰 로그**: 화면마다 `docs/pilot-<screenId>.md`에 있다(12개). 잔여 발견은 전부 `docs/BACKLOG.md`에 있고, 미룬 것에는 **트리거**가 달려 있다.
...
**조직 갈래를 열었다**(ORG-00, 27번째 화면). 셸의 '조직 관리' 갈피가 이제 켜져 있다.
남은 여덟은 ORG-03A·03B·03C · ORG-04·04B · ORG-07A·07B·07C다.

조직 갈래가 여섯 남았다가 셋이 됐다: 03A·03B·03C(조직도)와 04·04B(역할)를
마쳤다. 어휘는 다섯 늘었고(`itemList.emptyAction`·`titleField`·`itemMove`·
`itemRemove`, `button.action`의 `copy`) **04·04B에서는 한 줄도 안 늘었다** —
새 어휘를 만들기 전에 짝 화면을 열어 본 것이 쌌다.

**조직 갈래를 다 닫았다**(ORG-00·03A·03B·03C·04·04B·07A·07B·07C 아홉). 어휘가
여섯 늘었다: `itemList`의 `emptyAction`·`titleField`·`itemMove`·`itemRemove`·
`rowToneField`, `button.action`의 `copy`, 그리고 `screen.overlay`.

**`overlay`는 두 번째 화면에서 손대지 않고 섰다.** ORG-07C를 만들며 스키마도
검증기도 한 줄 안 고쳤다 — 모달을 처음 본 자리에서 정한 두 조각(어느 화면 위에
뜨는가, 이 화면이 그리는 부분이 어디인가)이 충분했다는 뜻이다.

**회의 계열을 열었다.** 첫 물음은 크기가 아니라 **셈법**이었다 — 와이어프레임의
회의 목록 넷(01A~01D)은 화면 넷이 아니라 **화면 하나와 변형 셋**이다. 사람이 그
사이를 오갈 수 없고, 누가 보느냐가 정한다.

어휘 `screen.variantOf`가 나왔다. 이 계열에 변형이 더 있다 —
03A/03B/03C(예정 회의, 역할 셋) · 05A/05B · 06A/06B.

**다음은 OPS-MEET-02(회의 생성·수정)다.** 01C의 '새 회의 만들기'가 그리로 가고
(지금은 pending), 그 뒤가 03A~03C다. 회의의 **상태**(예정→진행→정리→완료)를
명세가 어떻게 말할지가 그 다음 물음이다.

**눈금을 화면마다 잠갔다.** 총합 재현율은 회귀 탐지기가 아니었다 — 하루에 세 번
내렸다. 이제 `tests/screen-draft.test.mjs`의 `FLOOR`에 화면마다 줄이 있고,
**새 화면을 명세하면 그 줄을 더해야 검사가 지나간다.** ORG-03C에서 처음 일했다 —
막으면서 더할 줄을 그대로 알려 줬다.

ORG-00의 카드 셋은 지금 전부 pending이다. 각 대상이 명세되면 그때 잇는다
(EVT-FIN-01의 카드를 요청 상세로 이은 것과 같은 방식).

**아래는 재정 갈래를 닫았을 때의 판단이며 아직 유효하다.**

후보를 큰 것부터.

후보를 큰 것부터.

1. **회의 계열(19개)** — 가장 큰 덩어리인데 아직 OPS-MEET-01A 하나만 봤다.
   회의는 진행 중 상태가 있고 회의록이 따로 있어 재정과 성격이 다르다.
3. **기록(3개)** — REC-01·02·02A. 완료된 행사의 아카이브다.
4. **메시지(3개)·외부 참여(5개)** — 지금까지와 성격이 가장 다르다.
   외부 참여는 로그인하지 않은 사람이 보는 화면이라 **셸이 없을 수 있다.**

**회의 계열(19개)을 권한다.** 가장 큰 덩어리이고 아직 OPS-MEET-01A 하나만 봤다.
회의는 진행 중 상태가 있고 회의록이 따로 있어 지금까지의 어느 갈래와도 다르다 —
`steps`가 서버 상태로 쓰이는 자리가 다시 나올 것이고, 실시간에 가까운 상태를
명세가 어떻게 말할지가 새 물음이다.

**그 뒤로 미뤄 둔 것**

- **재정 보드의 네 열이 어디로 가는지** — 지금은 넷 다 요청 상세로 간다.
  검토는 FIN-REV-01, 구매는 FIN-PROC-01로 가야 할 것 같은데 **기획 물음**이다.
- **재정부가 보완 사유를 적는 자리가 없다** — 명세가 반쪽인 자리이고 그림이 없다.
- **추출기가 되풀이를 못 본다** — 사례가 넷이 됐다.
- **명세는 말하는데 아무도 안 읽는 것 셋** — 데이터 출처의 `loading`·`error`
  문구 122개, `fieldSet.kind`, `pending`을 어떻게 그릴지. 앞의 둘은 BACKLOG에
  있고, 셋째는 **표현을 정할 사람이 없는 자리**다(design에 대응하는 그림이
  없다 — pending은 개발 중에만 있는 것이기 때문이다).
- **3회차 교차 검증은 어휘가 또 늘면.** 1·2회차 사이가 아홉이었고, 2회차 뒤로
  지금까지 넷이 늘었다(`itemList.itemsField` · `executeWhen.sourceAllows` ·
  `screen.activeNavigationScreenId` · `group.headerFields`).
