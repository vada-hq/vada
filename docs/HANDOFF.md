...
| 계약이 든 자리 | 219 |
| **서버가 답하는 자리** | **142** (`apps/api/src/app.test.ts`가 래칫으로 지킨다) |
| **화면이 부르는 자리 중 남은 것** | **70** (`node scripts/left.mjs`) |
| 화면이 읽는 출처 중 진짜 | **120 / 173** · 쓰기 **20 / 46** |
| 검사 | 계약 214 · 앱 939 · api 819 · 서버 통합 157 · e2e 428 · **카나리 59**(화면 57장을 배포 모양으로 걷는다) |
| 검증 | 오류 0건 · 경고 0건 |
| 배포 | **돈다** — `vada.vadahq-ops.workers.dev` · 구글 로그인까지 |
| 배포된 앱이 그리는 화면 | **58 / 74** — 나머지 16장은 `이 화면은 아직 준비 중입니다`(그중 셋은 메시지, 여섯은 재정) |
...
```
밖에서    0    ✅        회의    9
들어오기  0    ✅        기록   21   ← 아카이브 발행을 정해야 연다
조직      2              재정   28   ← 예산 답이 나와야 연다
                         행사   10
```
