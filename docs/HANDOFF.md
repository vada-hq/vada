...
- **명세**: ONB-01·ONB-02·ORG-01·ORG-02·INV-01·HOME-01K·MY-01 7개 완결. 검증 오류 0건 경고 0건.
...
MY-01(내 업무) 사이클을 마쳤다(`docs/pilot-my01.md`). 두 번째 데스크톱 화면이라
HOME-01K에서 처음 연 대시보드 어휘를 처음으로 다시 시험한 화면이다.
...
다만 성격이 다르다. HOME-01K의 셋은 새 어휘였고(무엇을 표현할 수 없었다),
MY-01의 넷 중 셋은 **이미 있는 것을 잇는 자리**다 — `itemList.params`는
`select.optionsSource.params`와 같은 모양이고, `itemList.itemAction`은
`button.action`과 같은 규칙이며, `select.optionCounts`는 `dataSourceKey`를
하나 더 가리킬 뿐이다. 새 개념은 `shell.json` 하나다.

되풀이해 자라는 축은 **"화면의 값이 서버에서 온다"**의 갈래다. HOME-01K가
`data-sources.json`으로 열었고 MY-01이 세 방향으로 넓혔다(조회 인자·항목별
동작·선택지 곁의 값). **다음 화면에서 이 셋이 그대로 쓰이고 새 자리가 필요
없으면 그것이 첫 수렴 신호다.**

탭은 새 유형을 만들지 않았다 — `select` + `presentation: choiceGroup`이다.
근거는 `docs/pilot-my01.md` 3절에 있다.

**다음 후보**

1. **세 번째 데스크톱 화면**(운영 OPS-00 또는 재정) — 수렴을 판정하는 유일한
   길이다. 대시보드 어휘 셋이 새 자리 없이 쓰이는지가 관건이다.
2. **셸의 자산 자리** — 사이드바 아이콘이 화면마다 중복 저장된다. 지금은 셸을
   글자만으로 그린다. 데스크톱 화면이 하나 더 늘면 세 벌이 된다.
