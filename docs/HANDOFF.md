...
- **테스트**: spec-service·변환기·검증 96, vada-web(vitest) 211 + Playwright e2e 72 — 전부 통과. e2e는 AI가 직접 실행·스크린샷 판독하는 시각 검증 1차 수단이다(`apps/vada-web`에서 `npm run e2e`).
...
- **명세**: ONB-01·ONB-02·ORG-01·ORG-02·INV-01·HOME-01K·MY-01·OPS-00·TASK-01·OPS-MEET-01A·EVT-00A·EVT-02·EVT-TASK-01·EVT-TASK-02·EVT-DOC-01·EVT-MEET-01·EVT-SCHED-01 17개 완결. 검증 오류 0건 경고 0건.
...
EVT-SCHED-01(행사 일정) 사이클을 마쳤다(`docs/pilot-evt-sched-01.md`).
**행사 작업 공간의 다섯 번째 화면**이고, 앞 사이클에서 추출기를 크게 고친 뒤 처음
만나는 화면이라 **추출기의 첫 실측**이기도 했다.
...
- **추출기가 화면 하나를 통째로 맞혔다** — 처음이다. 눈금은 맞춤 67/103 · 헛것 11.
- **새 어휘는 없다.** `packages/contracts`를 한 줄도 손대지 않았다.
- **신규 계급 1건**은 대조기가 우연히 맞는 자리를 고른 것이다(§ 현재 상태).
  연속 0건은 여기서 끊긴다 — 조건 (1)은 EVT-DOC-01·EVT-MEET-01에서 한 번 관측됐고,
  다시 세려면 여기서부터다.

**다음 후보**

1. **행사 작업 공간의 남은 갈피 둘**(`EVT-04` 인원 관리 · `EVT-FIN-01` 재정) — 일곱
   중 다섯이 찼다. 둘을 채우면 EVT-TASK-01의 '아직 명세되지 않은 갈피' 검사가
   없어진다(지금까지 문서→관련 회의→일정→인원 관리로 옮겨 왔다).
2. **게이트 실행을 쪼갠다** — 벽시계가 예산(60초) 언저리에서 흔들린다. 바뀐 경로가
   깨뜨릴 수 있는 앱만 돌리는 것이 상수가 적어 둔 처방이다.
3. **행사 목록에서 개요로 잇는다**(`EVT-00A`의 '행사 상세 보기' pending) — 작은 일이다.
