...
시스템 보완 3건 완료: 검증 pre-commit 훅(장치화), 완성 판정 기준(방법론), action·완료 이벤트 계약 방향 확정(element-types — submit·mutations 카탈로그·scopeEvent, 스키마는 첫 제출 버튼 등장 시). 다음은 ORG-01(또는 INV-00) 사이클을 **표현력 시험**으로 진행한다 — 목록·제출이 등장할 확률이 높아 요소 유형 체계와 예약된 action 계약의 첫 증거를 공급할 것이다. 사용자가 Figma에서 해당 화면을 등록·저장하면 이후(정규화→구현→e2e)는 AI가 진행한다. 잔여 소소한 일: Figma 쪽 ONB-02 사본 동기화(`로컬 초안 불러오기`→`이 화면 저장` 1회).
