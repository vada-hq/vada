...
| 계약이 든 자리 | 222 |
| **서버가 답하는 자리** | **178** (`apps/api/src/app.test.ts`가 래칫으로 지킨다) |
| **화면이 부르는 자리 중 남은 것** | **39** (`node scripts/left.mjs`) |
| 화면이 읽는 출처 중 진짜 | **140 / 175** · 쓰기 **36 / 47** |
| 검사 | 계약 214 · 앱 951 · api 1,028 · 서버 통합 193 · e2e 428 · **카나리 60**(화면 58장을 배포 모양으로 걷는다) |
| 검증 | 오류 0건 · 경고 0건 |
| 배포 | **돈다** — `vada.vadahq-ops.workers.dev` · 구글 로그인까지 |
| 배포된 앱이 그리는 화면 | **62 / 75** — 나머지 13장은 `이 화면은 아직 준비 중입니다`(재정 7 · 메시지 3 · 회의 목록 1 · 아카이브 체크리스트 1 · 홈 재정 1) |
...
조직      1              재정   28   ← 예산 편성 화면이 섰다. 이제 그 위에 짓는다
                         행사    3   ← 행사 재정 셋(재정과 함께)
```
...
**뼈대부터 갔다**(2026-09-05~06). 자리 하나나 둘만 모자란 화면부터 세웠더니 하루에 준비 중이 40장에서 13장이 됐다. 남은 자리 대부분(재정 28)은 예산 편성 화면이 들어와 이제 열 수 있다 — 다음 회차는 거기서 시작한다.
