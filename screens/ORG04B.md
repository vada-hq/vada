...
**바뀐 명단은 변경 응답에서 읽는다.** `PUT`이 바뀐 전체 명단을 돌려주므로(`API:organization.change_member_role@R1`) 성공 후 목록을 다시 읽지 않는다. 다시 읽으면 같은 왕복을 한 번 더 한다 — 개발용 데이터베이스(싱가포르)에서 재보니 왕복 하나가 1.5초였고, 변경이 3초 걸리던 원인의 절반이 이 두 번째 읽기였다.

거절당했을 때만 다시 읽는다. 그때는 화면이 본 것이 낡았다는 뜻이므로 정본의 **충돌(409)** 항목이 요구하는 그대로다.
