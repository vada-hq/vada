...
**독립 화면이 아니라 팝업이다.** `내 구매 요청`(MY-REQ-01)과 `행사 재정`(EVT-FIN-01)의 오른쪽 위 `새 구매 요청`이 연다. 여는 화면은 뒤에 그대로 남고, 닫으면 그 화면으로 돌아온다.

경로 `/events/$eventId/purchase-requests/new`는 남긴다. 새로고침·뒤로가기·링크 공유가 되어야 하기 때문이다. 그 경로가 그리는 것은 **여는 화면인 내 구매 요청이고, 그 위에 팝업을 얹는다.** 팝업만 단독으로 그리지 않는다.

팝업 안은 **좌우 2단**이다. 왼쪽이 입력, 오른쪽이 고정 요약 패널이다. 머리말은 팝업 머리말이 갖고 본문만 스크롤한다.

**왼쪽 — 입력 영역**
1. **초안 복원 배너** — 서버 초안이 있을 때만. 저장 시각과 "제출 전에는 재정부 검토 목록에 표시되지 않습니다" 안내, 초안 삭제 행동.
2. **기본 요청 정보 카드** — 2열 격자로 요청 제목, 요청 부서(서버 값 고정·비활성), 필요한 날짜, 우선순위. 그 아래 전체 너비로 구매 목적.
3. **품목 리스트** — 머리에 "품목 리스트"와 총 품목 수, 오른쪽에 품목 추가. 품목마다 카드 하나:
