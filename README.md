...
Figma 화면을 JSON 명세로 만들고, 그 계약으로 VADA 웹과 API를 구현·검증·배포하는 모노레포다.
...
figma-spec-v2/
├─ apps/
│  ├─ api/             # [제품 vada] Hono API·인증·Postgres 저장소
│  ├─ spec-service/    # [파이프라인] Figma 수신·초안·검증·계약 생성 CLI
│  └─ vada-web/        # [제품 vada] 명세로 구현하는 React 프론트엔드와 Worker
├─ packages/
│  └─ contracts/       # [파이프라인] JSON 형식과 검증 규칙의 유일한 원본
│     └─ schemas/      # 사람이 직접 관리하는 JSON Schema 원본
├─ docs/
│  └─ decisions/       # 설계 결정 기록 (접두 없음=파이프라인, <제품>-*.md=제품)
├─ specs/figma/         # [wireframe] 화면·데이터·변이 계약과 Figma 원본
├─ scripts/             # 전체 검사·E2E·canary·운영 smoke 실행기
├─ tests/               # [파이프라인] 구성 요소 사이의 통합 검증
└─ .github/workflows/   # CI 게이트와 배포 후 smoke
```
...
```powershell
npm run check     # test + validate + e2e + canary
npm test          # 세 앱의 테스트만
```
...
API 타입은 `npm run openapi`로 OpenAPI·요청 스키마와 함께 생성한다. 서버가 구현할 동작은 [`api-type-operations.json`](specs/figma/vada-wireframe/api-type-operations.json)에서 관리하며, 생성한 [`api-types.d.ts`](specs/figma/vada-wireframe/api-types.d.ts)의 `ApiResponse`를 서버 반환과 웹 응답 해석에 함께 사용한다. 공통 변환기는 입력받은 카탈로그만 OpenAPI로 바꾸고, VADA 실행기가 제품 경로·문서 정보·세션 쿠키·출력을 지정한다. 계약 검사가 원본 카탈로그에서 다시 생성한 결과와 비교하므로, 명세를 바꾸고 산출물 갱신을 빠뜨리면 CI에서 실패한다.
...
빌드 검사 `vite-screen-boundary.ts`는 초기 JS와 그 정적 의존성에 실제 화면 구현이 들어가면 실패한다. 운영 브라우저 검사는 로그인에서 다른 화면 파일을 요청하지 않는지 확인하고, 실제 요청한 공통·로그인 JS의 gzip 합계를 170KB 미만으로 제한한다. `npm run smoke`는 Vite가 생성한 `assets/bundle-manifest.json`을 읽어 분리된 JS도 빠짐없이 검사한다.
...
`generate-screen-runtime.mjs`는 전체 화면 명세에서 라우팅·조회·초안 변환에 필요한 실행 메타데이터와 `screen-specs/`의 화면별 모듈을 생성한다. 라우터는 공통 메타데이터만 읽고, 실제 화면은 자기 명세를 화면 코드와 같은 지연 청크에서 불러온다. 전체 명세를 한꺼번에 읽는 `screen-catalog.ts`는 테스트에서만 사용한다.
