...
//
// **글자로 찾는다는 한계가 있다.** 열쇠가 값으로 오는 자리는 이 눈금이 못 본다 —
// 셸의 메뉴가 그렇다(`shell.json`이 열쇠를 들고 화면은 그것을 그대로 넘긴다).
// 그런 자리는 검사가 단추를 누르는 것과 **계약의 길을 곧장 부르는 것을 둘 다** 둔다:
// 앞엣것이 진짜 길이고, 뒤엣것이 여기에 보이는 증거다.
