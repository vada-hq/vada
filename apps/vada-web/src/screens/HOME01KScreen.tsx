...
export function HOME01KScreen({ onNavigate }: HOME01KScreenProps): ReactNode {
  ...
  return (
    <NavigateContext.Provider value={onNavigate}>
      ...
      >
        {/*
          **자리마다 따로 가린다.** 이 화면은 일곱을 읽는다. 한동안 그 하나(재정 요약)가
          예산을 정하는 화면이 없어 비어 있었고, 그 하나 때문에 화면이 통째로 닫히면
          나머지 여섯을 아무도 못 봤다. 이제 일곱이 다 서버에서 오지만 그물은 그대로
          둔다 — 하나가 다시 빠지는 날 그 자리만 가린다. 블록이 저마다 읽으므로
          감싸기만 하면 된다.
        */}
