...
export const DEVIATIONS: Deviation[] = [
  ...
  // --- EXT-02B: 안내 사항이 몇 줄인지는 데이터가 정한다 -----------------------
  //
  // **어휘가 없어서 그리지 못한다.** survey.applyResult.notices는 줄의 배열이고,
  // 몇 줄인지는 그 행사가 문의처를 적었는지가 정한다. 지금 명세에는 요소 하나가
  // 최상위에서 글줄의 배열을 그릴 자리가 없다 — 표도 목록도 아닌, 문단의 나열이다.
  //
  // OPS-MEET-06A·EVT-05B에 이어 **세 번째**다. 어휘를 만들 자리가 됐지만, 만들면
  // 이 커밋이 화면 다섯이 아니라 어휘 하나짜리가 된다. 침묵을 여기 적어 둔다.
  {
    by: 'place',
    screenId: 'EXT-02B',
    content: '· 신청 내용은 마감 전까지 운영진에게 문의하면 수정 가능합니다.',
    kind: '글 없음',
    design: 'gray-500(#6A7282)',
    screen: '화면에 없음',
    why:
      '안내 사항은 데이터가 주는 줄의 배열인데(survey.applyResult.notices) 그것을 ' +
      '최상위에서 그릴 어휘가 명세에 없다. 화면이 두 줄을 손으로 적으면 문의처를 ' +
      '적지 않은 행사에도 그 줄이 뜬다.',
  },
  {
    by: 'place',
    screenId: 'EXT-02B',
    content: '· 문의: @sw_student_council (인스타그램)',
    kind: '글 없음',
    design: 'gray-500(#6A7282)',
    screen: '화면에 없음',
    why: '위와 같다 — 몇 줄인지는 행사가 정하지 화면이 정하지 않는다.',
  },
  {
    by: 'place',
    screenId: 'EXT-02B',
    content:
      '안내 사항 · 신청 내용은 마감 전까지 운영진에게 문의하면 수정 가능합니다. · 문의: @sw_student_council (인스타그램)',
    kind: '칸 없음',
    design: '배경 gray-50(#F9FAFB) / 테두리 지정 없음',
    screen: '화면에 없음',
    why: '그 줄들을 담던 상자다. 안이 비면 상자도 없다.',
  },
