...
export function getSchemaPropertyEditorKind(schema, propertyKey) {
  const property = schema?.properties?.[propertyKey];

  if (!property) {
    throw new TypeError(`JSON Schema에 ${propertyKey} 속성이 없습니다.`);
  }
  ...
  if (property.type === "array") {
    return property.readOnly === true ? "empty-array" : "json-array";
  }
  ...
  const isString =
    ...
    (Array.isArray(property.type) &&
      ...
      property.type.every((type) => type === "string" || type === "null"));

  return isString ? "text" : "pending";
}

export function getSchemaTextPropertyKeys(schema) {
  return getSchemaPropertyKeys(schema).filter(
    (propertyKey) =>
      getSchemaPropertyEditorKind(schema, propertyKey) === "text"
  );
}
...
export function createNodeDraftStore() {
  const draftsByNodeId = new Map();

  return {
    save(nodeId, draft) {
      draftsByNodeId.set(nodeId, {
    ...
    load(nodeId) {
      const draft = draftsByNodeId.get(nodeId);
    ...
    remove(nodeId) {
      return draftsByNodeId.delete(nodeId);
    },

    entries() {
      return [...draftsByNodeId].map(([nodeId, draft]) => ({
        nodeId,
        elementType: draft.elementType,
        values: { ...draft.values }
      }));
