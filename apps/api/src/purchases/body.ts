...
//
// **한동안 두 꼴이 왔다.** 화면은 초안(상태 스코프)을 평평한 맵으로 들고 있고
// (`items: 'r0\nr1'`과 `items.r0.itemName`) 계약(OpenAPI)은 줄의 배열이라 적었는데, 그
// 사이를 옮기는 자리가 없어 화면마다 아무 꼴로나 보냈다. 그래서 이 파일은 둘 다 읽기로
// 했고, 배열만 읽던 서버 하나에서 그 어긋남이 터졌다 — ORG-02의 부서가 그것이다
// (2026-09-09). **어느 쪽이 계약인지 아무도 정하지 않은 채 파일마다 따로 정했던 것이다.**
//
// 지금은 정해졌다. 보내는 자리 하나가 계약의 꼴로 옮기고
// (`vada-web/src/spec/draft-values.ts`), 나가는 몸통을 계약에 대고 재며
// (`vada-web/src/spec/request-shape.ts`), 어긋나면 보내기 전에 던진다. **계약은 배열이고
// 그것 하나다.**
//
// 그래도 아래 평평한 갈래는 남긴다. 앱은 브라우저에 있고 사람은 옛 판을 연 채로 있을 수
// 있다 — 배포가 갈리는 동안 옛 화면이 보내는 것을 서버가 못 읽으면 그 사람은 저장을
// 잃는다. 예산 편성(`finance/budget-plan.ts`)과 회의 만들기(`meetings/create.ts`)가 같은
// 규칙을 지킨다.
