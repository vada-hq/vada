...
//
// ## 조건 목록은 그림이 든다 — 그중 셋은 아직 못 짓는다
//
// 명세의 조각은 모양만 말한다(묶음·줄·색·갈 곳). 무엇을 채워야 하는지는 EVT-05가
// **열여섯 줄로 그려 두었고**, 그중 열셋은 표가 답할 수 있다. 남은 셋은 아니다.
//
// - **이름 필수 문항 · 학번 필수 문항** — 어느 문항이 이름이고 어느 것이 학번인지를
//   표가 모른다. `survey_questions`가 든 것은 제목·갈래·필수·잠김뿐이고, 제목이
//   '이름'인지로 가르는 것은 운영진이 적은 글에 규칙을 거는 일이다.
// - **학생회비 대조용 식별 문항** — 같은 사정이다. 어느 문항이 대조에 쓰이는지를
//   적어 두는 자리가 없다.
//
// 문항에 **구실을 적는 열**이 생기는 날 세 줄이 함께 선다. 그때까지는 지어내지 않고
// 뺀다 — 없는 근거로 빨간 줄을 그리면 사람은 채울 수 없는 것을 채우러 다닌다.
