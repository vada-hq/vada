 ...
 * 비워 두고 기다린다.
 *
 * **읽는 쪽은 이 표를 안 본다**(2026-09-06). 장부(FIN-LEDGER-01)와 최근 지출은
 * 결제(`payments`)와 아직 안 낸 승인액에서 읽는다 — 계약이 '지출 예정' 줄을
 * 요구하는데 이 표는 **쓴 날이 있는 줄**만 담아 아직 안 쓴 것을 담을 수가 없다.
 *
 * 그러니 지금 이 표는 **비어 있고 아무도 안 읽는다.** 요청을 거치지 않은 상시 지출이
 * 들어오는 날 둘 중 하나를 해야 한다: 이 표에 예정을 담을 자리를 두고 읽는 쪽을
 * 이리로 옮기거나, 이 표를 지우고 결제 쪽에 상시 지출을 담거나. 그때까지 **살아
 * 있는 표로 오해하지 않도록** 여기 적어 둔다.
 */
