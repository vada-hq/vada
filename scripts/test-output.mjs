...
// 2026-08-31에 그 대가를 치렀다 — 워커가 죽어 아무것도 못 돈 것을 게이트가
...
export function ranNothing(output) {
  if (/^\s*Tests\s+no tests/m.test(output)) return true;
  const vitest = /^\s*Tests\s+\d+/m.test(output);
  const nodeTest = /^# pass \d+/m.test(output);
