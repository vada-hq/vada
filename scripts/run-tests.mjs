...
// "종료 훅의 자동 게이트가 120초를 넘기면 실패를 통과시킨다". 2026-08-30에 찾아보고
// "그런 장치가 이 저장소 어디에도 없다"고 적었다. `.githooks`가 pre-commit과 pre-push
// 둘뿐이고 시간 제한이 없는 것은 맞다.
//
// **그런데 그 장치는 있었다(2026-09-09에 말을 걸어 왔다).** 저장소가 아니라 도구 쪽에
// 산다 — Claude Code의 Stop 훅(`~/.claude/hooks/stop_gate.py`)이 `TIMEOUT_SEC = 120`으로
// `npm test --silent`를 돌린다. 저장소 안만 뒤졌으므로 못 찾았다.
//
// **방향이 위험한 쪽이다.** 시간을 넘기면 실패시키는 것이 아니라 **통과로 처리한다**
// (경고만 남기고 종료를 막지 않는다). 즉 스위트가 120초를 넘기기 시작하면 그때부터
// **안 돌린 것이 초록으로 읽힌다.** 아래가 막으려는 것이 "재는 저울이 느려지면 사람이
// 덜 재게 된다"인데, 실제로는 그보다 나쁘다 — 느려진 저울이 **거짓 초록을 낸다.**
//
// 그러니 아래 규칙은 그대로 옳고 근거만 바뀐다. 벽시계로 코드를 떨어뜨리지 않는 것은
// 여전히 맞다. 다만 '그런 장치가 없어서'가 아니라, **있고 조용히 초록을 내주기 때문에**
// 이 파일이 스스로 재고 스스로 말해야 한다.
//
...
//
// **그런데 이것도 결국 벽시계였다(2026-08-31).**
//
// 같은 코드로 두 번 재니 **86.5초와 119.8초**가 나왔다. 그 안을 보면 `tests`는
// 88.78 / 87.60으로 붙어 있는데 `environment`가 51초에서 344초로 흔들린다 —
// jsdom을 세우는 값이고, 그것은 남은 메모리가 0.4GB인 이 기계가 정한다.
//
// 그리고 그 흔들림이 **초록 코드의 푸시를 막았다**(120.7초, 검사는 전부 통과).
// 이 파일이 위에 적어 둔 말이 그대로 일어난 것이다 — 기계 때문에 코드를 떨어뜨리는
// 게이트는 늑대가 왔다고 거짓으로 외치는 것이다. 그 원칙이 정작 이 자리에만
// 적용되지 않고 있었다.
//
// **그래서 시간으로는 실패시키지 않는다.** 넘치면 크게 알리되 막지는 않는다.
// 막는 저울이 필요하면 시간이 아니라 **되풀이의 수**여야 한다 — 값을 줄인 처방
// 둘이 그것을 말해 준다(아래). 그 저울을 만드는 일은 BACKLOG에 있다.
//
...
` +
    `**막지는 않습니다.** 이 수는 기계가 절반을 정합니다(같은 코드로 86.5초와 119.8초).
`;
  record(`
>>> 시간 예산 초과
${reason}`);
