...
const FLOOR = {
  ...
  // 헛것 24 → 25 (2026-09-06). 행사별 예산 항목에 담당 부서 칸이 붙었고, 추출기는
  // 되풀이되는 줄의 칸을 줄마다 따로 세므로 등록한 것 하나에 헛것 하나가 는다.
  "FIN-PLAN-01": { matched: 13, spurious: 25 },
  ...
  // 7 → 6 (2026-09-06). **추출기가 나빠진 것이 아니라 등록이 옳아진 것이다.**
  // '내 담당 업무' 카드는 그림에서 눌리는 프레임이라 추출기가 button으로 뽑는데,
  // 그 부제('진행 중·검토 필요 4건')는 고정 글이 아니라 **데이터**다. button의 어휘에는
  // 데이터를 담을 자리가 없어 그 수가 명세에 굳었고, 업무가 없는 학생회의 홈이 4건이라
  // 말했다. summary + action으로 다시 등록하면서 추출기의 짐작과 갈렸다.
  //
  // **추출기가 못 가르는 것이 남는다**: 데이터가 든 눌리는 카드와 그냥 단추. 그것을
  // 가릴 근거는 그림이 아니라 계약에 있다(그 글이 출처의 조각인가). 그 자리가
  // 백로그에 있다.
  "HOME-01K": { matched: 6, spurious: 0 },
  "INV-01": { matched: 8, spurious: 0 },
  "MY-01": { matched: 3, spurious: 0 },
  // 초안이 그림을 그대로 읽었다(2026-09-09). 열하나를 뽑았고 헛것이 없었다 — 이
  // 화면은 초안을 먼저 뽑고 그 물음에 답해 지었기 때문이다.
  //
  // 맞춤이 일곱인 것은 추출기가 나빠져서가 아니라 **명세가 초안과 다르게 정했기
  // 때문**이다. 초안은 소속 카드를 `group` 하나와 `input` 셋으로 읽었는데, 그 셋은
  // 고치는 칸이 아니라 서버가 준 값이라 `summary` 하나로 적었다. 그림에 상자가 없는
  // 것이 그 사실을 이미 말하고 있었다 — 추출기가 라벨과 값을 보면 칸으로 읽는다.
  //
  // 헛것 셋이 바로 그 셋이다. 추출기가 틀린 것이 아니라 **그림만으로는 가를 수 없는
  // 것**이다: 상자 없는 라벨-값 쌍이 서버가 준 값인지 사람이 못 고치는 칸인지는
  // 무엇이 그 값을 정하느냐가 정하고, 그것은 그림 밖의 사실이다.
  "MY-INFO-01": { matched: 7, spurious: 3 },
  ...
  "EVT-05": { matched: 4, spurious: 7 },
  "EVT-05B": { matched: 4, spurious: 0 },
  "FIN-00": { matched: 1, spurious: 4 },
  "FIN-00B": { matched: 0, spurious: 7 },
  "FIN-LEDGER-01": { matched: 0, spurious: 0 },
  "MSG-01": { matched: 0, spurious: 1 },
  "MSG-02": { matched: 2, spurious: 5 },
  "MSG-03": { matched: 0, spurious: 1 },
  "INV-00": { matched: 1, spurious: 0 },
  ...
  "OPS-MEET-06A": { matched: 0, spurious: 2 },
  ...
  "OPS-MEET-D02": { matched: 2, spurious: 10 },
  "OPS-MEET-D03": { matched: 2, spurious: 7 },
