...
test("초안 재현율이 떨어지지 않는다(등록된 화면 전부)", async () => {
  ...
  // 2026-08-26 기준: 등록 194개 중 맞춤 106, 헛것 69.
  //
  // FIN-EVID-01은 9개 중 3개를 맞히고 헛것이 2다. 헛것 둘은 둘째·셋째 결제의
  // '파일 추가' 버튼이다 - 명세는 첫 사본만 등록하는데 추출기는 사본을 모르므로
  // 셋을 다 뽑는다. 되풀이를 못 보는 그 계급의 네 번째 사례다.
  //
  // 2026-08-26(앞): 등록 185개 중 맞춤 103, 헛것 67.
  //
  // FIN-PROC-01은 3개 중 2개를 맞히고 **헛것이 0**이다. 지금까지 중 가장 좋다.
  // 등록이 셋뿐이라 쉬웠던 것이 아니라, 묶음으로 오는 목록이 표 셋을 그리는데도
  // 추출기가 그것을 세 요소로 뽑지 않았다 - 되풀이를 못 보는 것이 여기서는
  // 오히려 맞았다. 못 맞힌 하나는 요약 카드이고, 그 어휘가 추출기에 없다.
  //
  // 2026-08-26(앞): 등록 182개 중 맞춤 101, 헛것 67.
  //
  // FIN-REV-01은 11개 중 1개를 맞혔다. 표 안에서 고치는 첫 화면이라 그렇다 -
  // 추출기는 표를 못 보고(알려진 계급), 표의 칸에 든 입력과 고르기를 화면 수준
  // 요소로 뽑는다. 헛것 다섯도 그 줄들이다.
  //
  // 2026-08-26(앞): 등록 171개 중 맞춤 100, 헛것 62.
  //
  // FIN-SUP-01은 9개 중 2개를 맞히고 헛것이 4다. 헛것 넷은 **칸 목록이 데이터에서
  // 오는 묶음**의 칸들이다 — 디자인에 네 칸이 그려져 있으니 추출기는 그것을 네
  // 요소로 뽑는데, 명세는 그 넷을 알지 않는다(fieldSet 하나로 적는다). 추출기가
  // 틀린 것이 아니라 **디자인만 보고는 그 넷이 고정인지 데이터인지 알 수 없다.**
  // 사람이 답해야 하는 것의 새 갈래이고, 헛것을 줄이는 길은 질문을 늘리는 것이다.
  //
  // 2026-08-26(앞): 등록 162개 중 맞춤 98, 헛것 58.
  //
  // MY-REQ-01은 6개 중 1개만 맞혔다. 못 맞힌 다섯 중 넷은 표와 요약이라 이미
  // 알려진 계급이고(추출기가 표를 못 본다), 하나는 **글 없는 조작**이다 —
  // 뒤로 가기 화살표에는 라벨이 없어서 추출기가 무엇의 조작인지 판정하지
  // 못하고 질문으로 돌린다. 지어내지 않고 물은 것이므로 옳은 동작이다.
  //
  // 2026-08-26(앞): 등록 156개 중 맞춤 97, 헛것 57.
  //
  // FIN-REQ-02는 5개 중 3개를 맞히고 **헛것이 0**이다. 못 맞힌 둘은 진행 단계 줄과
  // 요약 카드인데, 둘 다 추출기에 그 어휘가 없다(단계는 이번에 생겼다). 없는 것을
  // 지어내지 않고 조용히 지나간 것이므로 헛것이 늘지 않았다.
  //
  ...
  // 맞힌 첫 사례다.
  //
  // **헛것이 11 → 14 → 16으로 늘었다.** 나빠진 것이 아니라 화면을 더할 때마다
  // 이미 알려진 계급이 되풀이되는 것이다 — EVT-04의 셋(둘짜리 고르기 둘과 쪽 줄),
  // EVT-FIN-01의 둘(머리 버튼 짝, 화면 안 갈피 둘). 눈금을 늘려 잠그는 것은
  // 그것이 조용히 하나 더 늘어나는 것을 막기 위해서다.
  assert.ok(
    matched >= 106,
    `등록 ${registered}개 중 ${matched}개만 재현했습니다(106 이상이어야 함): ${worse.join(", ")}`
  );
  assert.ok(
    spurious <= 69,
    `등록되지 않은 요소를 ${spurious}개 뽑았습니다(69 이하여야 함): ${worse.join(", ")}`
  );
});
