...
test("초안 재현율이 떨어지지 않는다(등록된 화면 전부)", async () => {
  ...
  // 2026-08-25 기준: 등록 115개 중 맞춤 73, 헛것 14.
  ...
  // 맞힌 첫 사례다.
  //
  // **헛것이 11 → 14로 늘었다.** 나빠진 것이 아니라 EVT-04이 헛것 셋을 새로 낸
  // 것이고, 셋 다 이미 알려진 계급이다: 둘짜리 고르기 둘(상태 줄의 행동 짝,
  // 화면 안 갈피 둘)과 쪽 줄을 고르기로 읽은 것 하나. 눈금을 늘려 잠그는 것은
  // 그 셋이 조용히 넷이 되는 것을 막기 위해서다.
  assert.ok(
    matched >= 73,
    `등록 ${registered}개 중 ${matched}개만 재현했습니다(73 이상이어야 함): ${worse.join(", ")}`
  );
  assert.ok(
    spurious <= 14,
